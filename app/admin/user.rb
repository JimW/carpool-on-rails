...
show do
  attributes_table do
    ...
    # row :subscribe_to_gcal, as: :radio, collection: [["Yes", true], ["No", false]]
  ...
  panel "Carpools" do
    table_for user.driver_memberships do
      column "As Carpool Driver" do |carpool|
        li carpool.title#_for_admin_ui
      end
    end if user.driver_memberships.any?

    table_for user.passenger_memberships do
      column "As Passenger" do |carpool|
        li carpool.title#_for_admin_ui
      end
    end if user.passenger_memberships.any?
  end
...
form do |f|
  f.inputs "Details" do
    ...
    f.input :can_drive, :value => false, :as => :radio, :hint => "changing will remove all driver/passenger assignments"

    f.input :subscribe_to_gcal,
            :value => false,
            :as => :radio,
            :hint => "disabling will cause deletion of all personalized google carpool calendars" if current_user.is_admin? || resource == current_user
...
controller do # ______________________________ controller ______________________
  ...
  def update(options={}, &block)
		# p "HEY carpool_roles: " + params[:user][:carpool_role_ids].to_s
